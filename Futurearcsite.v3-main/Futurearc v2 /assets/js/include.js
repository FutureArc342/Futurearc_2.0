...
function initializePageScripts() {
    ...
    // --- LOGIK FÖR "KURSER LANSERAS SNART" POPUP ---
    const kurserPopup = document.getElementById('kurser-popup');
    if (kurserPopup) {
        let isKurserPopupVisible = false;
        let kurserPopupTimer;
        const hideKurserPopup = () => {
            if (!isKurserPopupVisible) return;
            clearTimeout(kurserPopupTimer);
            kurserPopup.classList.remove('active');
            setTimeout(() => { isKurserPopupVisible = false; }, 300);
        };
        const showKurserPopup = (event) => {
            event.preventDefault();
            if (isKurserPopupVisible) return;
            isKurserPopupVisible = true;
            kurserPopup.classList.add('active');
            kurserPopupTimer = setTimeout(hideKurserPopup, 3000);
        };
        // Lägg till event listeners för kurser-länkar
        document.querySelectorAll('a[href="#"][onclick*="showKurserPopup"]').forEach(link => {
            link.addEventListener('click', showKurserPopup);
        });
        // Stäng popup på klick utanför
        kurserPopup.addEventListener('click', (event) => {
            if (event.target === kurserPopup) hideKurserPopup();
        });
    }
